const MEETING_SERVICE_ENDPOINT = process.env.REACT_APP_MEETING_ENDPOINT.replace('hostname', window.location.hostname);

class MeetingService {
  async fetchMeetingList() {
    const resp = await fetch(MEETING_SERVICE_ENDPOINT, {
      method: 'GET',
      headers: {
  ...
  async queryMeeting({applicationId, channelName}) {
    ...
    const resp = await fetch(MEETING_SERVICE_ENDPOINT + '/queryMeeting', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json;charset=UTF-8'
      },
  ...
  async queryMeetingById(meetingId) {
    const resp = await fetch(MEETING_SERVICE_ENDPOINT + '/' + meetingId, {
      method: 'GET',
      mode: 'cors',
      cache: 'default',
    });
